...
"SELECT * FROM alumno a
	      ...
	      ON a.no_cuenta=i.no_cuenta
          JOIN imparte im
          ON im.id_imparte=i.id_imparte
          JOIN asig_grupo ag
          ON ag.id_asigrupo=im.id_asigrupo
          JOIN semestre s
          ON s.id_semestre=im.id_semestre
	      LEFT JOIN lab_asig la
	      ON la.id_asig=ag.id_asig
          LEFT JOIN formato f
	      ON la.id_lab=f.id_lab
          left join practica_asig pa
          on pa.id_Asig=ag.id_asig
          WHERE a.no_cuenta='" .$login . 
		 " AND vigencia_actual=1
		   AND im.id_semestre=5
           AND (la.id_lab=3 OR la.id_lab=2 OR la.id_lab=1)"
